...
export default class Client extends CryptoConsumer {
  ...
  static async readTeacupNoteByName(
    noteName,
    encryptionKeyPair,
    signingKeyPair,
    realmName,
    ...
    console.log('READING TEACUP NOTE BY NAME')
    let anonAuth = await AuthenticatedRequest.anonymousAuth(
      this.crypto,
      signingKeyPair.publicKey,
      signingKeyPair.privateKey,
    ...
    // Use this to ensure we referencing the implementing class.
    // eslint-disable-next-line camelcase
    const params = { id_string: noteName, tozid_login_token_realm: realmName }
    const headers = { 'X-TOZID-LOGIN-TOKEN': toznySpyJWT }
    return _readNote(
      this.crypto,
      anonAuth,
      encryptionKeyPair,
