...
async function decryptNoteJson(
  ...
  let verify = await _verify(signed, publicSigningKey)
  if (!verify) {
 ...
 */
async function issueNoteChallenge(client, key, identifier) {
  if (client.config.version === 1) {
    throw new Error('Cannot read notes without a signing key!')
  }
  const validKey = key === 'id_string' ? 'id_string' : 'note_id'
  const request = await client.authenticator.tsv1Fetch(
    `${this.config.apiUrl}/v2/storage/notes/challenge?${validKey}=${identifier}`,
    {
      method: 'PATCH',
    }
  )
...
export default class Client extends CryptoConsumer {
   ...
   */
  async noteChallenge(noteId) {
    issueNoteChallenge(this, 'note_id', noteId)
  }
   ...
   * @return {Array.<string>} An array of strings for challenges issued.
   */
  async noteChallengeByName(noteName) {
    issueNoteChallenge(this, 'id_string', noteName)
  }
