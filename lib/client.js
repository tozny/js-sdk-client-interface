...
import { default as AuthRequest } from './auth'
...
import CoreNote from './types/coreNote'
...
const PreClient = crypto => {
  ...
  class Client {
    ...
    constructor(config) {
      ...
      this.authenticator = new AuthRequest(config)
    ...
    async sign(document) {
      ...
      return crypto.signDocument(document, this.config.privateSigningKey)
    }
    ...
    async localDecrypt(record, eak) {
      ...
      let verify = await Client.verify(signed, eak.signerSigningKey.ed25519)
      if (!verify) {
    ...
    async readNote(noteId) {
      ...
      return Client.internalReadNote(
        this.authenticator,
        { public: this.config.publicKey, private: this.config.privateKey },
        noteId,
        this.config.apiUrl
      )
    }
    ...
    async readNoteByName(noteName) {
      ...
      return Client.internalReadNoteByName(
        this.authenticator,
        { public: this.config.publicKey, private: this.config.privateKey },
        noteName,
        this.config.apiUrl
    ...
    /**
     * WriteNote encrypts a note and sends it to TozStore.
     *
     * @param {string} type  Type of note to write
     * @param {object} data  A hashmap of the data to encrypt and store
     * @param {object} plainMeta A hashmap of unencrypted tags for data
     * @param {object} fileMeta A hashmap of unencrypted tags for large files
     * @param {string} recipientSigningKey signing key of the reader of this note
     * @param {object} options A NoteOptions object, or a json hashmap of a NoteOptions object.
     *
     * @returns {Note} A response from TozStore; the note that has been written.
     */
    async writeNote(type, data, plainMeta, fileMeta, recipientSigningKey, options) {
      if (this.config.version === 1) {
        throw new Error('Cannot write notes without a signing key!')
      }
      return Client.internalWriteNote(
        this.authenticator,
        type,
        data,
        plainMeta,
    ...
    async deleteNote(noteId) {
      ...
      return Client.internalDeleteNote(this.authenticator, noteId, this.config.apiUrl)
    }
     ...
     *
     * @param {string} noteId  UUID assigned by TozStore, used to identify a note.
     * @param {SigningKeyPair} signingKeyPair Object that has signing public and private keys
     * @param {EncryptionKeyPair} encryptionKeyPair Object that has encryption public and private keys
    ...
    static async readNote(
      ...
      let anonAuth = AuthRequest.anonymousAuth(
        crypto,
        signingKeyPair.public,
        signingKeyPair.private
      )
      Client.internalReadNote(anonAuth, encryptionKeyPair, noteId, apiUrl)
    }
    ...
    static async readNoteByName(
      ...
      return Client.internalReadNoteByName(anonAuth, encryptionKeyPair, noteName, apiUrl)
    }
     ...
     * @param {string} recipientSigningKey signing key of the reader of this note
     * @param {SigningKeyPair} signingKeyPair Object that has signing public and private keys
     * @param {string} apiUrl Url of the TozStore api that you want to hit (Default is recommended).     * @param {EncryptionKeyPair} encryptionKeyPair Object that has encryption public and private keys
     *
     * @returns {Note} A response from TozStore; the note that has been written.
     */
    static async writeNote(
      ...
      return Client.internalWriteNote(
        ...
        recipientSigningKey,
        encryptionKeyPair,
        {}, // Options not provided for anonymous notes
        apiUrl
      )
    }
    ...
    static async deleteNote(noteId, signingKeyPair, apiUrl = DEFAULT_API_URL) {
      ...
      return Client.internalDeleteNote(anonAuth, noteId, apiUrl)
    }

    /*
     * Decrypts and validates a note response from TozStore given the proper keys.
     */
    static async decryptNoteJson(noteJson, publicSigningKey, privateKey, publicKey) {
      ...
      let signed = new SignedDocument(decryptedNote.core, decryptedNote.signature)
      let verify = await Client.verify(signed, publicSigningKey)
      if (!verify) {
     ...
     */
    static async internalReadNote(authenticator, encryptionKeyPair, noteId, apiUrl) {
      let storedNote = await authenticator.tsv1Fetch(
      ...
      return Client.decryptNoteJson(
        noteJson,
        authenticator.publicSigningKey,
    ...
    static async internalWriteNote(
      ...
      var noteOptions = NoteOptions.decode(options)
      let signature = await crypto.signDocument(coreNote, authenticator.privateSigningKey)
      var note = new Note(coreNote, signature, noteOptions)
      let encryptedNote = await crypto.encryptNote(note, accessKey)
      let response = await authenticator.tsv1Fetch(apiUrl + '/v2/storage/notes', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
