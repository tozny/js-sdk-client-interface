...
import { default as NoteInfo } from './types/noteInfo'
import { default as NoteOptions } from './types/noteOptions'
import NoteData from './types/noteData'
import NoteKeys from './types/noteKeys'
...
const PreClient = crypto => {
  ...
  class Client {
     ...
     */
    async writeNote(data, recipientSigningKey, options) {
      if (this.config.version === 1) {
    ...
    static async writeNote(
      ...
      var decodedOptions = NoteOptions.decode(options)
      let optionSubset = {
        type: decodedOptions.type,
        plain: decodedOptions.plain,
        file_meta: decodedOptions.fileMeta
      }
      return Client.internalWriteNote(
        anonAuth,
        data,
    ...
    static async internalWriteNote(
      ...
      let noteOptions = new NoteOptions.decode(options)
      var signableNote = new NoteInfo(noteData, noteKeys, noteOptions)
      let signature = await crypto.signDocument(
        signableNote,
        authenticator.privateSigningKey
      )
      var unencryptedNote = new Note(noteData, noteKeys, signature, noteOptions)
