...
const PreClient = crypto => {
  ...
  class Client {
    ...
    async readNote(noteId) {
      ...
      let storedNote = await this.authenticator.tsv1Fetch(
        this.config.apiUrl + `/v2/storage/notes?note_id=${noteId}`,
        {
          method: 'GET'
        }
      )
    ...
    async readNoteByName(noteName) {
      ...
      let storedNote = await this.authenticator.tsv1Fetch(
        this.config.apiUrl + `/v2/storage/notes?id_string=${noteName}`,
        {
          method: 'GET'
        }
      )
    ...
    async writeNote(type, data, plainMeta, fileMeta, recipientSigningKey, options) {
      ...
      let signature = await this.sign(coreNote)
    ...
    async deleteNote(noteId) {
      ...
      let deletedNoteResponse = await this.authenticator.tsv1Fetch(
        this.config.apiUrl + `/v2/storage/notes/${noteId}`,
        {
          method: 'DELETE'
        }
      )
    ...
    static readNote(noteId) {}

    static readNoteByName(noteName) {}

    static writeNote(type, data, plainMeta, fileMeta, recipientSigningKey) {}
