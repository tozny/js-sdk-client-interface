...
export default class Client {
  /**
   * Abstract crypto instance which must be made concrete by the implementing code.
   *
   * When creating a client with a concrete crypto instance, extend this class and overload this getter to offer up a
   * valid, concrete implementation of the Crypto interface. No other overloading is necessary unless desired.
   *
   * example:
   *
   * ```
   ...
   * static method is called, this is bound to the class constructor used, which will be the implementing class. It
   * is odd to see `this` in static methods, however this is what allows for late static biding. In other words,
   * the static methods defined here in the interface class can fetch and use methods defined in the crypto instance
   * provided byt the implementing child class by referencing `this` instead of referencing `Client` directly.
   *
   * An additional instance level getter is provided which allows fetching the crypto instance in both static _and_
   * instance method contexts as `this.crypto`.
   *
  ...
  static async readNote(
    ...
    return this.constructor.internalReadNote(anonAuth, encryptionKeyPair, noteId, apiUrl)
  }
  ...
  static async internalWriteNote(
    ...
    let noteOptions = NoteOptions.decode(options)
  ...
  static async internalReadNote(authenticator, encryptionKeyPair, noteId, apiUrl) {
    ...
    // Use this.constructor to ensure we referencing the implementing class.
    return this.constructor.decryptNoteJson(
      noteJson,
      authenticator.publicSigningKey,
      encryptionKeyPair.privateKey,
      encryptionKeyPair.publicKey
  ...
  /**
   * Gets the static crypto object so it can be used with `this.crypto` in instance methods.
   *
   * This is a helper to allow `this.crypto` access within instance methods.
   *
