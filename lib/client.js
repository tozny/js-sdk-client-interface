 ...
 * @returns {Promise}
 */
async function authenticatedFetch(client, urlString, options) {
  let authHeader = client.auth.authenticate(urlString, options)
  options.headers = options.headers || {}
  options.headers.Authorization = authHeader
  return fetch(urlString, options)
}

async function tokenFetch(client, urlString, options) {
  let authHeader = client.auth.getTokenHeader()
  options.headers = options.headers || {}
  options.headers.Authorization = authHeader
  return fetch(urlString, options)
}

async function signatureFetch(client, urlString, options) {
  let authHeader = client.auth.getSignatureHeader(urlString, options)
  options.headers = options.headers || {}
  options.headers.Authorization = authHeader
  return fetch(urlString, options)
}
...
const PreClient = crypto => {
  ...
  class Client {
    ...
    constructor(config) {
      ...
      this.config = config
      this.authenticator = new Auth(config.apiKeyId, config.apiSecret, config.publicSigningKey, config.privateSigningKey, crypto, config.apiUrl)
    ...
    async read(recordId, fields = null) {
      ...
      let request = await tokenFetch(this, path, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        }
    ...
    async writeRaw(record) {
      ...
      let request = await tokenFetch(this, this.config.apiUrl + '/v1/storage/records', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
    ...
    async delete(recordId, version = null) {
      ...
      let response = await tokenFetch(this, url, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        }
     ...
     */
    async _query(query) {
      let response = await tokenFetch(this, this.config.apiUrl + '/v1/storage/search', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
    ...
    async writeLargeFile(recordType, fileObject, plainMetadata = {}) {
      ...
      const postRequest = await tokenFetch(
        this,
      ...
      const patchRequest = await tokenFetch(
        this,
    ...
    async readLargeFile(recordId, destinationFilename) {
      ...
      */
      ...
      let getRequest = await tokenFetch(
        this,
