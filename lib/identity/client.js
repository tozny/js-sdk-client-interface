...
export default class Client {
  ...
  async changePassword(username, oldPassword, newPassword) {
    ...
    const { oldCredentials, backupOldCredentials } = await this.backupOldPassword(
      username,
      oldPassword
    )
    ...
    await this._storageClient.writeNote(
      oldCredentials.data,
    ...
    // Remove backup credentials
    await this._storageClient.deleteNote(backupOldCredentials.noteId)
  }

  async backupOldPassword(username, oldPassword) {
    const oldCreds = await deriveNoteCreds(
      this.config,
      this._crypto,
      username,
      oldPassword
    )
    const oldCredentials = await this._storageClient.constructor.readNoteByName(
      oldCreds.noteID,
      oldCreds.cryptoKeyPair,
      oldCreds.signingKeyPair,
      this.config.apiUrl
    )
    // Remove old backup if it exists
    try {
      const oldBackupCredentials = await this._storageClient.constructor.readNoteByName(
        oldCreds.noteID + '-backup',
        oldCreds.cryptoKeyPair,
        oldCreds.signingKeyPair,
        this.config.apiUrl
      )
      await this._storageClient.deleteNote(oldBackupCredentials.noteId)
    } catch (error) {
      // Throw away error if old backup was properly removed
    }
    // Save credential backup
    let backupOldCredentials = await this._storageClient.writeNote(
      oldCredentials.data,
      oldCreds.cryptoKeyPair.publicKey,
      oldCreds.signingKeyPair.publicKey,
      {
        id_string: oldCreds.noteID + '-backup',
        max_views: -1,
        expires: false
      }
    )
    await this._storageClient.deleteNote(oldCredentials.noteId)
    return { oldCredentials, backupOldCredentials }
  }
