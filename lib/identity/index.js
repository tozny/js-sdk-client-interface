...
export default class Identity extends CryptoConsumer {
  ...
  async login(username, password) {
    ...
    console.log(
      `[login] after derived note creds ${noteID}, ${cryptoKeyPair}, ${signingKeyPair}`
    )
    ...
    console.log(`[login] after storedCreds ${storedCreds}`)
    const idConfig = Config.fromObject(storedCreds.data.config)
    console.log(`[login] after idconf ${idConfig}`)
    /* eslint-disable-next-line no-warning-comments */
