...
export default class Identity extends CryptoConsumer {
  ...
  // METHOD DOES NOT REGISTER BROKER NOTES FOR SIMPLICITY
  async teacupRegister(username, password, token, realmName, email) {
    const cryptoKeys = await this.StorageClient.generateKeypair()
    const signingKeys = await this.StorageClient.generateSigningKeypair()
    /* eslint-disable camelcase */
    const payload = {
    ...
    await idClient.storageClient.writeNote(
      ...
      signingKeyPair.publicKey,
      {
        ...
        expires: false,
        eacp: {
          tozid_eacp: { realm_name: realmName }, // CREATE WITH TEACuP - may not match spec
        },
      }
    )
    return idClient
  ...
  async teacupLogin(
    ...
    const storedCreds = await this.StorageClient.readTeacupNoteByName(
      noteID,
      cryptoKeyPair,
      signingKeyPair,
      realmName,
