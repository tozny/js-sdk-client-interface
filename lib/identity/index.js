...
import { IDENTITY_DERIVATION_ROUNDS } from '../utils/constants'
...
export default class Identity extends CryptoConsumer {
  ...
  fromObject(obj) {
    ...
    if (!obj.config) {
      throw new Error(
        'To create an identity client form an object it must contain identity configuration'
      )
    }
    if (!obj.storageConfig) {
      throw new Error(
        'To create an identity client form an object it must contain storage configuration'
      )
    }
    /* eslint-disable-next-line no-warning-comments */
    // TODO: Validate creds match this realm before instantiating
