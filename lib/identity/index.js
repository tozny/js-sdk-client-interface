...
export default class Identity extends CryptoConsumer {
  ...
  async initiateBrokerLogin(
    username,
    brokerUrl = `${this.config.apiUrl}/v1/identity/${this.config.realmName}/broker/challenge`
  ) {
    const payload = {
      username,
      action: 'challenge',
  ...
  async completeBrokerLogin(
    authResponse,
    noteId,
    brokerUrl = `${this.config.apiUrl}/v1/identity/${this.config.realmName}/broker/login`
  ) {
